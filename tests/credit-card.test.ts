...
describe('[Credit Card]', () => {
    ...
    const testValue0 = creditCard();
    test(`Should ${clrExpected('begins with 34/37 and 15 digits long')} by default [ ${clrValue(testValue0)} ]`, () => {
        expect(testValue0).toMatch(/^3(4|7)\d( \d{4}){3}$/);
    });
