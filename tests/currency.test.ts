...
describe('[Currency]', () => {
    ...
    const testValue1 = currency({ separator: ',', decimals: 2, min: 100, max: 150, formats: [{ symbol: '$' }] });
    ...
    const testValue2 = currency(({ separator: ',', decimals: 3, min: 1, max: 500000, formats: [{ symbol: '₹' }] }));
    ...
    const testValue3 = currency({ separator: '', decimals: 0, min: 2000, max: 3000, formats: [{ symbol: '₽' }] });
